...
class PowerDNSContentHandler(AbstractContentHandler):
    ...
    def _filter_records(self, qtype_records):
        ...
        for qtype in qtype_records:
            lb_method = 'p'
            if len(qtype_records[qtype]) > 0 and 'lbmethod' in qtype_records[qtype][0] and qtype_records[qtype][0][
                'lbmethod'] != None:
    ...
    def _lb_wrr(self, records):
        ...
        if sum_weight > 0:
            ...
            for record in records:
                if not self._is_in_view(record) or record['fallback']:
                    continue
                if record['id'] not in powergslb.monitor.get_status():
                    proba_max += float(record['weight']) / float(sum_weight)
                    logging.debug(
                        "live_records - sum_weight: %d - rand: %f - record['weight']: %d - proba_max = w+rw/s: %f",
                        sum_weight, rand, record['weight'], proba_max)
                    if proba_max >= rand:
        ...
        if sum_weight_fallback > 0:
            ...
            for record in records:
                ...
                logging.debug(
                    "fallback_records - sum_weight: %d - rand: %f - record['weight']: %d - proba_max = w+rw/s: %f",
                    sum_weight, rand, record['weight'], proba_max)
                if proba_max >= rand:
