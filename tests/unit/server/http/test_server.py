...
def test_init_reads_plain_config(status_registry: StatusRegistry) -> None:
    thread = HTTPServerManager(_config(), {'host': 'db'}, status_registry, PowerDNSRequestHandler, name='Server')
...
def test_init_defaults_root_to_bundled_resources(status_registry: StatusRegistry) -> None:
    config = _config()
    del config['root']
    thread = HTTPServerManager(config, {}, status_registry, PowerDNSRequestHandler, name='Admin')
...
def test_run_tls_without_ciphers_skips_set_ciphers(
    ...
    thread = HTTPServerManager(_config(ssl=True, cert='/c.pem'), {}, status_registry,
                               PowerDNSRequestHandler, name='Admin')
...
def test_init_tls_without_cert_raises(status_registry: StatusRegistry) -> None:
    ...
    with pytest.raises(ValueError, match='certificate'):
        HTTPServerManager(_config(ssl=True), {}, status_registry, PowerDNSRequestHandler, name='Admin')
